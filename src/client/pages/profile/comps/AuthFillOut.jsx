...
function AuthFillOut() {
    ...
    const [formData, setFormData] = useState({ username: '', currentPassword: '', newPassword: '', confirmNewPassword: '' });


    useEffect(() => {
      if (me) {
        setFormData({ username: me.username || '', currentPassword: '', newPassword: '', confirmNewPassword: '' });
      }
    }, [me]);
    ...
    if (editError) return <div>Error: {editError.message}</div>;
      ...
      <div className="flex justify-center items-center min-h-screen bg-gray-200 p-5">
        ...
        <form className="bg-white p-8 rounded-lg w-full sm:w-4/5 md:w-3/4 lg:w-2/3 xl:w-1/2" onSubmit={handleSubmit}>
          ...
          <div className="flex justify-center">
            <button type="submit" className="relative w-full p-2 text-white-600 bg-blue-500 border-2 border-indigo-600 rounded-full shadow-2xl transition-all duration-500 ease-out text-white  overflow-hidden group hover:bg-blue-gray-700"
>
              Update Profile
            </button>
          </div>
